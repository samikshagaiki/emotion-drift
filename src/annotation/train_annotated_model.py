...
X_PATH = os.path.join(DATA_DIR, "X_train_annot.npy")
Y_PATH = os.path.join(DATA_DIR, "y_train_annot.npy")
GROUPS_PATH = os.path.join(DATA_DIR, "groups_annot.npy")

MODEL_KERAS_PATH = os.path.join(PROJECT_DIR, "eye_emotion_model_true5.keras")
MODEL_H5_PATH = os.path.join(PROJECT_DIR, "eye_emotion_model_true5.h5")
REPORT_JSON_PATH = os.path.join(REPORT_DIR, "true5_metrics.json")
CM_PATH = os.path.join(REPORT_DIR, "true5_confusion_matrix.csv")
...
def main():
    if not (os.path.exists(X_PATH) and os.path.exists(Y_PATH) and os.path.exists(GROUPS_PATH)):
        raise FileNotFoundError(
            "Annotated dataset not found. Run create_annotation_template.py and "
            "build_annotated_dataset.py first."
        )
    ...
    print("Annotated distribution:", dict(sorted(dist.items())))
    print("Shapes:", X.shape, y.shape, groups_1d.shape)
    ...
    callbacks = [
        tf.keras.callbacks.ModelCheckpoint(
            MODEL_KERAS_PATH, monitor="val_loss", save_best_only=True, mode="min", verbose=1
        ),
    ...
    report = classification_report(
        y_test, y_pred, labels=list(range(NUM_CLASSES)), output_dict=True, zero_division=0
    )
